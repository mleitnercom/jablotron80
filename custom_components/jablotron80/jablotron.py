...
# Time-based, ack-independent safety-net sweep (issue #153 follow-up).
# On a setup with many detectors open at once, the panel's "?" detail-query
# response can overflow and never get acked, so the query-ack-gated
# reconciliation in _update_device() never runs and a detector the panel has
# STOPPED reporting (e.g. a closed door) stays stuck active=True in HA. The
# panel's status-text reports ("Triggered detector, X:name") DO come through
# reliably, so we record the last time each device was reported active and
# deactivate any active device not re-reported within DEVICE_STALE_TIMEOUT_SECONDS.
# This sweep ONLY deactivates; it never activates. 120s is ~3 worst-case
# status-text cycles - sized to avoid a false "closed" while still clearing a
# genuinely-closed detector.
            ...
            if not prev == cur:
                if prev is None:
                    ...
                    if args[0].__class__.__name__ == "JablotronSensor" and _id == 1:
                        ...
                        LOGGER.debug(
                            f"{args[0].__class__.__name__}({_id}): {var_name} changed from {prev} to {cur}"
                        )
                    else:
                        LOGGER.info(
                            f"{args[0].__class__.__name__}({_id}): {var_name} changed from {prev} to {cur}"
...
class JablotronDevice(JablotronCommon):
    ...
    def is_keyfob(self) -> bool:
        return self.model == "RC-86" or self.model == "RC-86 (80)"
...
class JablotronZone(JablotronCommon):
    ...
    def device_activated(self, device: JablotronDevice) -> None:
        if (
            ...
            or device.reaction == JablotronConstants.REACTION_FIRE_ALARM
        ):
            # self.alarm(device)
            pass
    ...
    def code_activated(self, code: JablotronCode) -> None:
        if (
            self.status == JablotronZone.STATUS_ARMED
            or code.reaction == JablotronConstants.REACTION_FIRE_ALARM
        ):
            # self.alarm(code)
            pass
...
class JablotronMessage:
    ...
    def get_length(main_type: str, record: bytes) -> int:
        ...
        if length is None:
            if main_type in [
                JablotronMessage.TYPE_BEEP,
                JablotronMessage.TYPE_KEYPRESS,
                JablotronMessage.TYPE_PING,
            ]:
                return 2
            else:
                # settings
    ...
    def check_crc(packet: bytes) -> bool:
        ...
        crcchecker = crccheck.crc.Crc(
            8, 70, initvalue=49, xor_output=0, reflect_input=False, reflect_output=False
        )
    ...
    def get_message_type_from_record(record, packet_data: bytes) -> str:
        ...
        if message_type is None:
            ...
            if not JablotronMessage.check_crc(record):
                ...
                LOGGER.log(
                    expected_warning_level,
                    f"Invalid message of type {message_type} received {packet_data}",
                )
        return None
...
class JablotronState:
    ...
    STATES_ALARM = [
        ...
        ALARM_B,
        ALARM_C,
        ALARM_WITHOUT_ARMING,
        ALARM_A_SPLIT,
        ALARM_B_SPLIT,
        ALARM_C_SPLIT,
        ALARM_WITHOUT_ARMING_SPLIT,
    ]
    ...
    STATES_ENTERING_DELAY = [
        ARMED_ENTRY_DELAY_A,
        ARMED_ENTRY_DELAY_AB,
        ARMED_ENTRY_DELAY_ABC,
        ARMED_ENTRY_DELAY_A_SPLIT,
        ARMED_ENTRY_DELAY_B_SPLIT,
        ARMED_ENTRY_DELAY_C_SPLIT,
    ]
...
class JA80CentralUnit(object):
    ...
    def __init__(
        ...
        # Drives the ack-independent staleness sweep (_sweep_stale_devices).
    ...
    def devices(self) -> List[JablotronDevice]:
        ...
        return list(
            filter(
                registered,
                [self.get_device(i) for i in range(1, MAX_NUMBER_OF_DEVICES)],
            )
        )
    ...
    def zones(self) -> List[JablotronZone]:
        return sorted(
            [zone for zone in self._zones.values() if zone.enabled],
            key=lambda zone: zone._id,
        )

    @property
    def codes(self) -> List[JablotronCode]:
        return sorted(
            [code for code in self._codes.values() if code.enabled],
            key=lambda code: code._id,
        )
    ...
    def _activate_device(self, source):
        if isinstance(source, JablotronDevice):
            ...
            # #153 follow-up: record the moment this device was last reported
            # active so the staleness sweep can deactivate it if the panel
            # stops reporting it (runs for every "Triggered detector, X" report).
    ...
    def _sweep_stale_devices(self) -> list:
        # #153 follow-up: ack-independent safety net. Deactivate any active
        # device that has not been re-reported active for longer than
        # DEVICE_STALE_TIMEOUT_SECONDS. This NEVER activates a device; it only
        # turns off detectors the panel has silently stopped reporting (e.g. a
        # closed door whose "?" query reconciliation never ran because the
        # detail-query response overflowed and was never acked).
        now = time.monotonic()
        deactivated = []
        for device in self._active_devices.values():
            if device.active is not True:
                continue
            # Default to ``now`` so a device with no recorded timestamp is
            # treated as just-seen and is NOT swept.
            last = self._device_last_active.get(device.device_id, now)
            age = now - last
            if age > DEVICE_STALE_TIMEOUT_SECONDS:
                LOGGER.info(
                    f"Detector {device.device_id} not reported for "
                    f">{DEVICE_STALE_TIMEOUT_SECONDS}s, marking inactive"
                )
                device.active = False
                # Keep the device in _active_devices so it can re-activate.
    ...
    async def _stale_device_sweep_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            changed = self._sweep_stale_devices()
    ...
    async def _process_state(self, data: bytearray, packet_data: str) -> None:
        ...
        self.led_power = (
            leds & 0x01
        ) == 0x01  # if this is not set, power is out on control panel and power led flashes
        ...
        if setting_type_1 == 0x02:
            ...
            if setting_type_2 == 0x00:
                ...
                if device_id >= 1 and device_id <= self._max_number_of_wired_devices:
                    ...
                    serial_hex_string = "".join(map(lambda x: hex(x)[2:], data[5:11]))
                    ...
                    if data[5:7] == b"\x05\x08" or data[5:7] == b"\x05\x09":
    ...
    async def send_keypress_sequence(
        ...
        await self._connection.add_command(
            JablotronCommand(
                ...
                code=value,
                accepted_prefix=accepted_prefix + b"\xff",
